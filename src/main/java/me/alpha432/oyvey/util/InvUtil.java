...
public class InvUtil implements Util {
    ...
    void switchToHotbarSlot ( int slot , boolean silent ) {
        ...
        if ( silent ) {
            InvUtil.mc.player.connection.sendPacket ( new CPacketHeldItemChange ( slot ) );
    ...
    List < Integer > findEmptySlots ( boolean withXCarry ) {
        ...
        if ( withXCarry ) {
            for (int i = 1; i < 5; ++ i) {
                Slot craftingSlot = InvUtil.mc.player.inventoryContainer.inventorySlots.get ( i );
    ...
    int findInventoryBlock ( Class clazz , boolean offHand ) {
        ...
        slot.set ( - 1 );
        for (Map.Entry < Integer, ItemStack > entry : InvUtil.getInventoryAndHotbarSlots ( ).entrySet ( )) {
            if ( ! InvUtil.isBlock ( entry.getValue ( ).getItem ( ) , clazz ) || entry.getKey ( ) == 45 && ! offHand )
                continue;
            slot.set ( entry.getKey ( ) );
            return slot.get ( );
        }
    ...
    Map < Integer, ItemStack > getInventorySlots ( int currentI , int last ) {
        ...
        for (int current = currentI; current <= last; ++ current) {
            fullInventorySlots.put ( current , InvUtil.mc.player.inventoryContainer.getInventory ( ).get ( current ) );
        }
    ...
    Map < Integer, ItemStack > fuckYou3arthqu4kev2 ( int currentI , int last ) {
        HashMap < Integer, ItemStack > fullInventorySlots = new HashMap <> ( );
        for (int current = currentI; current <= last; ++ current) {
            fullInventorySlots.put ( current , InvUtil.mc.player.openContainer.getInventory ( ).get ( current ) );
        }
        return fullInventorySlots;
    }
    ...
    boolean isSlotEmpty ( int i ) {
        Slot slot = InvUtil.mc.player.inventoryContainer.inventorySlots.get ( i );
    ...
    int findArmorSlot ( EntityEquipmentSlot type , boolean binding , boolean withXCarry ) {
        int slot = InvUtil.findArmorSlot ( type , binding );
