...
class SprintFuture
    ...
    void onUpdate ( ) {
        switch (this.mode.getValue ( )) {
            case RAGE: {
                if ( ! SprintFuture.mc.gameSettings.keyBindForward.isKeyDown ( ) && ! SprintFuture.mc.gameSettings.keyBindBack.isKeyDown ( ) && ! SprintFuture.mc.gameSettings.keyBindLeft.isKeyDown ( ) && ! SprintFuture.mc.gameSettings.keyBindRight.isKeyDown ( ) || SprintFuture.mc.player.isSneaking ( ) || SprintFuture.mc.player.collidedHorizontally || (float) SprintFuture.mc.player.getFoodStats ( ).getFoodLevel ( ) <= 6.0f )
                    break;
                SprintFuture.mc.player.setSprinting ( true );
                break;
            }
            case LEGIT: {
                if ( ! SprintFuture.mc.gameSettings.keyBindForward.isKeyDown ( ) || SprintFuture.mc.player.isSneaking ( ) || SprintFuture.mc.player.isHandActive ( ) || SprintFuture.mc.player.collidedHorizontally || (float) SprintFuture.mc.player.getFoodStats ( ).getFoodLevel ( ) <= 6.0f || SprintFuture.mc.currentScreen != null )
                    break;
                SprintFuture.mc.player.setSprinting ( true );
            }
        }
