...
public class ConfigManager implements Util {
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Xuluplus").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("Xuluplus/" + name + "/"))) {
            this.config = "Xuluplus/" + name + "/";
        } else {
            this.config = "Xuluplus/config/";
        }
