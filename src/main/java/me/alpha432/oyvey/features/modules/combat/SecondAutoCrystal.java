...
public class SecondAutoCrystal extends Module {

    private final me.alpha432.oyvey.util.Timer placeTimer = new me.alpha432.oyvey.util.Timer();
    private final me.alpha432.oyvey.util.Timer breakTimer = new me.alpha432.oyvey.util.Timer();
    private final me.alpha432.oyvey.util.Timer preditTimer = new me.alpha432.oyvey.util.Timer();
    private final me.alpha432.oyvey.util.Timer manualTimer = new me.alpha432.oyvey.util.Timer();
    ...
    public Setting<Float> placeDelay = this.register(new Setting<Float>("Delays", 4.0f, 0.0f, 300.0f));
    public Setting<Float> placeRange = this.register(new Setting<Float>("Range", 4.0f, 0.1f, 7.0f));
    ...
    public Setting<Boolean> silentSwitch = this.register(new Setting<Boolean>("Silent", true, v -> this.switchmode.getValue() == SwitchMode.Silent));
    ...
    private void rotateTo(Entity entity) {
        if (this.rotate.getValue().booleanValue()) {
            float[] angle = MathUtil.calcAngle(SecondAutoCrystal.mc.player.getPositionEyes(me.alpha432.oyvey.util.Util.mc.getRenderPartialTicks()), entity.getPositionVector());
            this.yaw = angle[0];
            this.pitch = angle[1];
            this.rotating = true;
        }
    ...
    private void rotateToPos(BlockPos pos) {
        if (this.rotate.getValue().booleanValue()) {
            float[] angle = MathUtil.calcAngle(SecondAutoCrystal.mc.player.getPositionEyes(me.alpha432.oyvey.util.Util.mc.getRenderPartialTicks()), new Vec3d((float) pos.getX() + 0.5f, (float) pos.getY() - 0.5f, (float) pos.getZ() + 0.5f));
            this.yaw = angle[0];
            this.pitch = angle[1];
            this.rotating = true;
        }
    ...
    public void onCrystal() {
        ...
        if (SecondAutoCrystal.mc.player.getHeldItemOffhand().getItem() != Items.END_CRYSTAL) {
            int crystalSlot;
            crystalSlot = SecondAutoCrystal.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL ? SecondAutoCrystal.mc.player.inventory.currentItem : -1;
            if (crystalSlot == -1) {
                for (int l = 0; l < 9; ++l) {
        ...
        if (this.target.getDistance(SecondAutoCrystal.mc.player) > 12.0f) {
            this.crystal = null;
            this.target = null;
        }
        ...
        if (this.crystal != null && this.explode.getValue() && this.breakTimer.passedMs(this.breakDelay.getValue().longValue())) {
            ...
            if (this.packetBreak.getValue()) {
                ...
                SecondAutoCrystal.mc.player.connection.sendPacket(new CPacketUseEntity(this.crystal));
            } else {
                this.rotateTo(this.crystal);
                SecondAutoCrystal.mc.playerController.attackEntity(SecondAutoCrystal.mc.player, this.crystal);
            }
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue()) {
            ...
            for (BlockPos blockPos : this.placePostions(this.placeRange.getValue())) {
                ...
                if (targetDmg < (double) this.minDamage.getValue() && (this.facePlaceSword.getValue() ? this.target.getAbsorptionAmount() + this.target.getHealth() > this.facePlace.getValue().floatValue() : SecondAutoCrystal.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword || this.target.getAbsorptionAmount() + this.target.getHealth() > this.facePlace.getValue().floatValue()) && (this.facePlaceSword.getValue() != false ? !this.armor : SecondAutoCrystal.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword || !this.armor) || (selfDmg = this.calculateDamage((double) blockPos.getX() + 0.5, (double) blockPos.getY() + 1.0, (double) blockPos.getZ() + 0.5, SecondAutoCrystal.mc.player)) + (this.suicide.getValue() != false ? 2.0 : 0.5) >= (double) (SecondAutoCrystal.mc.player.getHealth() + SecondAutoCrystal.mc.player.getAbsorptionAmount()) && selfDmg >= targetDmg && targetDmg < (double) (this.target.getHealth() + this.target.getAbsorptionAmount()) || !(damage < targetDmg))
                    continue;
                this.pos = blockPos;
                damage = targetDmg;
            }
            ...
            if (this.hotBarSlot != -1 && this.autoswitch.getValue() && !SecondAutoCrystal.mc.player.isPotionActive(MobEffects.WEAKNESS) && this.switchmode.getValue() == SwitchMode.Normal && !this.silentSwitch.getValue()) {
            ...
            int old = me.alpha432.oyvey.util.Util.mc.player.inventory.currentItem;
            EnumHand hand = null;
            if (switchmode.getValue() == SwitchMode.Silent) {
                if (slot != -1) {
                    if (me.alpha432.oyvey.util.Util.mc.player.isHandActive() && this.silentSwitch.getValue()) {
                        hand = me.alpha432.oyvey.util.Util.mc.player.getActiveHand();
                    }
                    me.alpha432.oyvey.util.Util.mc.player.connection.sendPacket(new CPacketHeldItemChange(slot));
                }
            }
            ...
            if (!this.ignoreUseAmount.getValue()) {
                ...
                if (this.crystalCount < crystalLimit && this.pos != null) {
                    ...
                    SecondAutoCrystal.mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(this.pos, EnumFacing.UP, SecondAutoCrystal.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
                }
            ...
            if (switchmode.getValue() == SwitchMode.Silent) {
                if (slot != -1) {
                    if (this.silentSwitch.getValue() && hand != null) {
                        me.alpha432.oyvey.util.Util.mc.player.setActiveHand(hand);
                    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof SPacketSpawnObject && (packet = event.getPacket()).getType() == 51 && this.predicts.getValue().booleanValue() && this.preditTimer.passedMs(this.attackFactor.getValue().longValue()) && this.predicts.getValue().booleanValue() && this.explode.getValue().booleanValue() && this.packetBreak.getValue().booleanValue() && this.target != null) {
            ...
            SecondAutoCrystal.mc.player.connection.sendPacket(predict);
        }
    }
    ...
    private boolean isPredicting(SPacketSpawnObject packet) {
        ...
        if (EntityUtil.isInHole(SecondAutoCrystal.mc.player) && targetDmg >= 1.0) {
            return true;
        }
        double selfDmg = this.calculateDamage(packet.getX() + 0.5, packet.getY() + 1.0, packet.getZ() + 0.5, SecondAutoCrystal.mc.player);
    ...
    private boolean IsValidCrystal(Entity p_Entity) {
        ...
        if (p_Entity.getDistance(SecondAutoCrystal.mc.player) > this.breakRange.getValue().floatValue()) {
            return false;
        }
        if (!SecondAutoCrystal.mc.player.canEntityBeSeen(p_Entity) && p_Entity.getDistance(SecondAutoCrystal.mc.player) > this.breakWallRange.getValue().floatValue()) {
            return false;
        }
        ...
        double selfDmg = this.calculateDamage((double) p_Entity.getPosition().getX() + 0.5, (double) p_Entity.getPosition().getY() + 1.0, (double) p_Entity.getPosition().getZ() + 0.5, SecondAutoCrystal.mc.player);
        double d = this.suicide.getValue() != false ? 2.0 : 0.5;
        if (selfDmg + d < (double) (SecondAutoCrystal.mc.player.getHealth() + SecondAutoCrystal.mc.player.getAbsorptionAmount()) && targetDmg >= (double) (this.target.getAbsorptionAmount() + this.target.getHealth())) {
            return true;
        }
    ...
    EntityPlayer getTarget() {
        ...
        for (EntityPlayer entity : SecondAutoCrystal.mc.world.playerEntities) {
            if (SecondAutoCrystal.mc.player == null || SecondAutoCrystal.mc.player.isDead || entity.isDead || entity == SecondAutoCrystal.mc.player || OyVey.friendManager.isFriend(entity.getName()) || entity.getDistance(SecondAutoCrystal.mc.player) > 12.0f)
                continue;
            this.armorTarget = false;
            ...
            if (!(closestPlayer.getDistance(SecondAutoCrystal.mc.player) > entity.getDistance(SecondAutoCrystal.mc.player)))
                continue;
            closestPlayer = entity;
        }
            ...
            if (result.typeOfHit.equals(RayTraceResult.Type.ENTITY)) {
                ...
                if (entity instanceof EntityEnderCrystal) {
                    if (this.packetBreak.getValue().booleanValue()) {
                        SecondAutoCrystal.mc.player.connection.sendPacket(new CPacketUseEntity(entity));
                    } else {
                        SecondAutoCrystal.mc.playerController.attackEntity(SecondAutoCrystal.mc.player, entity);
                    }
                    this.manualTimer.reset();
                }
            } else if (result.typeOfHit.equals(RayTraceResult.Type.BLOCK)) {
                BlockPos mousePos = new BlockPos(SecondAutoCrystal.mc.objectMouseOver.getBlockPos().getX(), (double) SecondAutoCrystal.mc.objectMouseOver.getBlockPos().getY() + 1.0, SecondAutoCrystal.mc.objectMouseOver.getBlockPos().getZ());
                for (Entity target : SecondAutoCrystal.mc.world.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB(mousePos))) {
                    ...
                    if (this.packetBreak.getValue().booleanValue()) {
                        SecondAutoCrystal.mc.player.connection.sendPacket(new CPacketUseEntity(target));
                    } else {
                        SecondAutoCrystal.mc.playerController.attackEntity(SecondAutoCrystal.mc.player, target);
                    }
                    this.manualTimer.reset();
                }
            }
    ...
    private boolean canSeePos(BlockPos pos) {
        return SecondAutoCrystal.mc.world.rayTraceBlocks(new Vec3d(SecondAutoCrystal.mc.player.posX, SecondAutoCrystal.mc.player.posY + (double) SecondAutoCrystal.mc.player.getEyeHeight(), SecondAutoCrystal.mc.player.posZ), new Vec3d(pos.getX(), pos.getY(), pos.getZ()), false, true, false) == null;
    }

    private NonNullList<BlockPos> placePostions(float placeRange) {
        NonNullList positions = NonNullList.create();
        positions.addAll(SecondAutoCrystal.getSphere(new BlockPos(Math.floor(SecondAutoCrystal.mc.player.posX), Math.floor(SecondAutoCrystal.mc.player.posY), Math.floor(SecondAutoCrystal.mc.player.posZ)), placeRange, (int) placeRange, false, true, 0).stream().filter(pos -> this.canPlaceCrystal(pos, true)).collect(Collectors.toList()));
        return positions;
    }
    ...
    private float calculateDamage(double posX, double posY, double posZ, Entity entity) {
        ...
        if (entity instanceof EntityLivingBase) {
            finald = this.getBlastReduction((EntityLivingBase) entity, this.getDamageMultiplied(damage), new Explosion(SecondAutoCrystal.mc.world, null, posX, posY, posZ, 6.0f, false, true));
        }
        return (float) finald;
    }
