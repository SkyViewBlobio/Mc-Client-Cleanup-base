...
public class LongJump extends Module {
    ...
    private final Setting <Float> boost = this.register ( new Setting <> ( "Boosted" , 4.48f , 1.0f , 20.0f ) );
    private final Setting <Mode> mode = this.register ( new Setting <> ( "Modes" , Mode.DIRECT ) );
    ...
    private final Setting <Boolean> disableStrafe = this.register ( new Setting <> ( "StrafeOff" , false ) );
    private final Setting <Boolean> strafeOff = this.register ( new Setting <> ( "StrafeUnoc" , false ) );
    ...
    void onEnable ( ) {
        ...
        if ( StrafePhob.getInstance ( ).isOn ( ) && this.disableStrafe.getValue ( ) ) {
    ...
    void onTickEvent ( final TickEvent.ClientTickEvent event ) {
        ...
        if ( StrafePhob.getInstance ( ).isOn ( ) && this.strafeOff.getValue ( ) ) {
            this.disable ( );
            return;
        }
    ...
    private void doNormal (final UpdateWalkingPlayerEvent event) {
        ...
        switch (this.mode.getValue ( )) {
            ...
            case DIRECT: {
                if ( EntityUtilPhob.isInLiquid ( ) || EntityUtilPhob.isOnLiquid ( ) ) {
                    break;
                }
