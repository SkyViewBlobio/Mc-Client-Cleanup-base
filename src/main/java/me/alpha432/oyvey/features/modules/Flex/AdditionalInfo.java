...
public class AdditionalInfo
    ...
    public AdditionalInfo() {
        super("AdditionalInfos", "HudInfos", Category.Flex, false, false, true);
    }
    ...
    public void drawTargetHud(float partialTicks) {
        if (this.design.getValue() == TargetHudDesign.NORMAL) {
            EntityPlayer target = AutoCrystal.target != null ? AutoCrystal.target : (Killaura.target instanceof EntityPlayer ? (EntityPlayer) Killaura.target : AdditionalInfo.getClosestEnemy());
            if (target == null) {
                return;
            }
            ...
            //Integer ping = XuluRenderUtilBoss.isFakePlayer(target) ? 0 : (mc.getConnection().getPlayerInfo(target.getUniqueID()) == null ? 0 : mc.getConnection().getPlayerInfo(target.getUniqueID()).getResponseTime());
    ...
    public static EntityPlayer getClosestEnemy() {
        ...
        for (EntityPlayer player : AdditionalInfo.mc.world.playerEntities) {
            ...
            if (!(AdditionalInfo.mc.player.getDistanceSq((Entity) player) < AdditionalInfo.mc.player.getDistanceSq((Entity) closestPlayer)))
                continue;
            closestPlayer = player;
        }
