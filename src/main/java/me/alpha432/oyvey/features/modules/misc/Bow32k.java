...
public class Bow32k extends Module {
    ...
    public Bow32k() { super("Bow32k", "One Shots Players", Category.MISC, true, false, false); }
    ...
    public Setting <Boolean> bypass = this.register ( new Setting <> ( "Bypasses", false));
    public Setting <Boolean> debug = this.register ( new Setting <> ( "DebugOnToggle", false));
        ...
        if (event.getPacket() instanceof CPacketPlayerDigging) {
            ...
            if (packet.getAction() == CPacketPlayerDigging.Action.RELEASE_USE_ITEM) {
                ...
                if (!handStack.isEmpty() && handStack.getItem() != null && handStack.getItem() instanceof ItemBow && Bows.getValue()) {
                    doSpoofs();
                    if (debug.getValue()) Command.sendMessage("Spoof ATTEMPT");
                }
            }
