...
class Scaffold extends Module {
    ...
    public
    Scaffold ( ) {
        super ( "Scaffold" , "TowersUpwards " , Category.MOVEMENT , true , false , false );
        this.mode = this.register ( new Setting <> ( "Modes" , Mode.Normal ) );
        this.rotation = this.register ( new Setting ( "Rotate" , false , ( v ) -> this.mode.getValue ( ) == Mode.Normal ) );
        this.swing = this.register ( new Setting ( "Swing(for2b)" , false , ( v ) -> this.mode.getValue ( ) == Mode.Vanilla ) );
        this.bSwitch = this.register ( new Setting ( "Switches" , false , ( v ) -> this.mode.getValue ( ) == Mode.Vanilla ) );
        this.center = this.register ( new Setting ( "CenterMyself" , false , ( v ) -> this.mode.getValue ( ) == Mode.Vanilla ) );
        this.keepY = this.register ( new Setting ( "KeepYLevel" , false , ( v ) -> this.mode.getValue ( ) == Mode.Vanilla ) );
        this.sprint = this.register ( new Setting ( "UseSprintFuture" , true , ( v ) -> this.mode.getValue ( ) == Mode.Vanilla ) );
        this.replenishBlocks = this.register ( new Setting ( "ReplenishBlocks" , true , ( v ) -> this.mode.getValue ( ) == Mode.Vanilla ) );
        this.down = this.register ( new Setting ( "Down" , false , ( v ) -> this.mode.getValue ( ) == Mode.Vanilla ) );
        this.expand = this.register ( new Setting ( "Expand" , 1.0F , 0.0F , 6.0F , ( v ) -> this.mode.getValue ( ) == Mode.Vanilla ) );
    ...
    void onUpdate ( ) {
        if ( this.mode.getValue ( ) == Mode.Vanilla ) {
            if ( OyVey.moduleManager.isModuleEnabled ( "SprintFuture" ) && ( (Boolean) this.down.getValue ( ) && mc.gameSettings.keyBindSneak.isKeyDown ( ) || ! (Boolean) this.sprint.getValue ( ) ) ) {
