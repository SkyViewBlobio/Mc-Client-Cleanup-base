...
public class AutoArmorPhob extends Module {
    ...
    private final Setting < Boolean > shiftClick = this.register ( new Setting <> ( "ShiftClick[CrashWhenSneak]" , false ) );
    ...
    void onTick ( ) {
        ...
        if ( this.taskList.isEmpty ( ) ) {
            ...
            ItemStack chest;
            int slot4;
            if ( this.mendingTakeOff.getValue ( ) && InventoryUtil.holdingItem ( ItemExpBottle.class ) && AutoArmor.mc.gameSettings.keyBindUseItem.isKeyDown ( ) && ( this.isSafe ( ) || EntityUtilPhob.isSafe ( AutoArmor.mc.player , 1 , false , true ) ) ) {
            ...
            if ( ( chest = AutoArmor.mc.player.inventoryContainer.getSlot ( 6 ).getStack ( ) ).getItem ( ) == Items.AIR ) {
                ...
                slot3 = InventoryUtil.findItemInventorySlot ( Items.DIAMOND_CHESTPLATE , false , XCarryPhob.getInstance ( ).isOn ( ) );
                if ( slot3 == - 1 && ( slot3 = InventoryUtil.findItemInventorySlot ( Items.IRON_CHESTPLATE , false , XCarryPhob.getInstance ( ).isOn ( ) ) ) == - 1 && ( slot3 = InventoryUtil.findItemInventorySlot ( Items.GOLDEN_CHESTPLATE , false , XCarryPhob.getInstance ( ).isOn ( ) ) ) == - 1 && ( slot3 = InventoryUtil.findItemInventorySlot ( Items.CHAINMAIL_CHESTPLATE , false , XCarryPhob.getInstance ( ).isOn ( ) ) ) == - 1 ) {
                    slot3 = InventoryUtil.findItemInventorySlot ( Items.LEATHER_CHESTPLATE , false , XCarryPhob.getInstance ( ).isOn ( ) );
                }
                if ( slot3 != - 1 ) {
                    this.taskList.add ( new InventoryUtil.Task ( slot3 ) );
                    this.taskList.add ( new InventoryUtil.Task ( 6 ) );
        ...
        if ( this.timer.passedMs ( (int) ( (float) this.delay.getValue ( ) * ( this.tps.getValue ( ) ? OyVey.serverManager.getTpsFactor ( ) : 1.0f ) ) ) ) {
            if ( ! this.taskList.isEmpty ( ) ) {
    ...
    private
    boolean isSafe ( ) {
        EntityPlayer closest = EntityUtilPhob.getClosestEnemy ( this.closestEnemy.getValue ( ) );
        if ( closest == null ) {
            return true;
        }
