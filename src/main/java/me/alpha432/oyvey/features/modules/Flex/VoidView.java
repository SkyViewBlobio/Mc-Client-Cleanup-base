...
class VoidView
    ...
    private final Setting < Integer > alpha = this.register ( new Setting <> ( "Saturation" , 255 , 0 , 255 ) );
    ...
    public
    VoidView ( ) {
        super ( "VoidView" , "views the void" , Category.Flex , true , false , false );
    }
    ...
    public
    void onTick ( ) {
        if ( ! ( VoidView.fullNullCheck ( ) || this.noEnd.getValue ( ) && VoidView.mc.player.dimension == 1 || ! this.timer.passedMs ( this.updates.getValue ( ) ) ) ) {
    ...
    void onRender3D ( Render3DEvent event ) {
        if ( VoidView.fullNullCheck ( ) || this.noEnd.getValue ( ) && VoidView.mc.player.dimension == 1 ) {
            return;
        }
    ...
    List < BlockPos > findVoidHoles ( ) {
        BlockPos playerPos = EntityUtil.getPlayerPos ( VoidView.mc.player );
    ...
    boolean isVoid ( BlockPos pos ) {
        return ( VoidView.mc.world.getBlockState ( pos ).getBlock ( ) == Blocks.AIR || ! this.air.getValue ( ) && VoidView.mc.world.getBlockState ( pos ).getBlock ( ) != Blocks.BEDROCK ) && pos.getY ( ) < 1 && pos.getY ( ) >= 0;
    }
}